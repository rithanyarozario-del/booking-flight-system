...
def init_db(): #Creates an booking table that does not already exist to seperate users bookings. 
...
def calculate_cost(departure, arrival, adults, children, bags):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("SELECT flight_code, dep_time, arr_time, base_fare FROM flights WHERE departure = ? AND arrival = ?", (departure, arrival))
    row = c.fetchone()
    conn.close()
    if row is None: 
        return None
    flight_code, dep_time, arr_time, base_fare = row
    adult_cost = int(adults) * base_fare
    child_cost = int(children) * base_fare * CHILD_DISCOUNT
    baggage_cost = int(bags) * BAGGAGE_FEE
    total_cost = round(adult_cost + child_cost + baggage_cost, 2)
    return {
...
def save_bookings(username, booking):
    ...
    cost = apply_date_surcharge(flight_info["total_cost"], booking.get("date"))
    ...
    try:
        c.execute("""
            INSERT INTO bookings (username, departure, arrival, date, passengers, adults, children, ticket, cost, bags, return_date, flight_code, )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            username,
            booking.get ("departure"),
